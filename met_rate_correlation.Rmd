...
d[,'body_temperature']=d$`typical_body_temperature [°C]`
...
sbs_rates_colnames=grep(paste(c('SBS','Substitution rate per year'), collapse="|"), names(mut_rates_all))
...
mut_rates_df[,c('Species','Substitution rate per year')]
```
...
for (n in 3:length(names(life_traits_list))){
  cols=life_traits_list[[names(life_traits_list)[n]]]
    for (y_val in c('log_lifespan','log_mass_specific_BMR','log_mutation_rate')){
...
partial_corr_colnames=c('log_lifespan','log_mutation_rate','log_respiratory_rate','log_heart_rate','log_adult_mass','log_time_to_sexual_maturity_female',
                        'log_time_to_sexual_maturity_male','litter_size','log_mass_specific_BMR')#'log_mass_specific_BMR')#,'log_basal_metabolic_rate')
                        
                        # These columns are either sparse, or irrelevant (body temperature)
                        #'body_temperature','log_mass_specific_BMR','log_Mitochondrial_respiration_heart','log_H2O2_production_heart')
...
var_to_regress_out=c('log_mass_specific_BMR')
...
my_cols=qualitative_hcl(length((vars_to_compare)),palette = "Dark 2") 
...
for (correlation_result_metric in result_names_list){
  ...
  colnames(melted_df)=c('Bootstrap_number','variable','value')
   ...
   p=ggplot(data=melted_df,aes(x = value, fill = variable,y=..density..)) +
            ...
            labs(x=correlation_result_metric)+
            theme(legend.position="none",
...
subplot_title <- ggdraw() + draw_label('Significance of partial correlation with lifespan after \ncorrecting for somatic mutation rate (500 resamples)',                                        size = 20,fontface='bold') #
...
d[,'y']=exp((0.63*1.602*10^-19)/((1.38*10^-20)*310))*(d[,'BMR (kcal/kg BW/day)'])*(d[,'Adult mass (g)']/1000)*(d$`Lifespan_80 (y) [Species 360/Human Mortality Database]`)*365*4184/(4.5*1000)
d[,'x']=0.001*exp((0.63*1.602*10^-19)/((1.38*10^-20)*303))*1.62*10^8*d[,'Adult mass (g)']/1000 
