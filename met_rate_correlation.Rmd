...
for (n in  3:length(names(life_traits_list))){ 
  cols=life_traits_list[[names(life_traits_list)[n]]]
    for (y_val in c('log_lifespan','log_mutation_rate','log_mass_specific_BMR')){ #'log_SBSC_rate','log_SBS1_rate','log_SBSB_rate'
      ...
      for (x_val in cols[!grepl(y_val,cols)]){
        ...
        if (pgls_fit_package=='ape'){
          ...
          lik <- sapply(lambda, function(lambda) logLik(gls(form,correlation = corPagel(value = lambda, phy = tree, 
                                                                                        fixed = T,form=~species_name),
...
for (n in 1:length(figure_names)){
  ...
  for (k in 1:length(colnames_to_loop)){
      ...
      subplot_title <- ggdraw() + draw_text(subplot_letter, x=0.05,y=0.5,size = 65,fontface='bold') + 
                       draw_label(title_string, size = 35,fontface='bold') 
...
partial_corr_colnames=c('log_lifespan','log_mutation_rate','log_respiratory_rate','log_heart_rate','log_adult_mass',
                        'log_time_to_sexual_maturity_female','log_time_to_sexual_maturity_male','litter_size','log_mass_specific_BMR')
                        #'log_mass_specific_BMR')#,'log_basal_metabolic_rate')
...
for (log_transform_p in (c(TRUE))){
  ...
  for (i in seq_along(result_names_list)){
    ...
    subplot_letter=subplot_letters[i]
    plot_df=data.frame()
    ...
    if (log_transform_p==TRUE){
        ...
        #x_label=gsub('p-value',paste0('-',expression(log[10]),'(p-value)'),correlation_result_metric)
    ...
    p=ggplot(data=melted_df,aes(x = value, fill = variable,y=..density..)) +
            ...
            # annotate("text", x = sign_level, y = 0.9, label = 'a', 
     ...
     #  p=p + annotate('text',x = x_coord, y = y_coord,label=paste('a'))
     # 
  ...
  subplot_title <- ggdraw() + draw_label(paste0('Significance levels of partial correlation with lifespan \nafter correcting for somatic mutation rate (',num_samples,' resamples)'),y=0.7,size = 14,fontface='bold') #+ 
                   #draw_text('a', x=0.03,y=0.0,size = 15,fontface='bold') +
                   #draw_text('b', x=0.43,y=0.0,size = 15,fontface='bold')
